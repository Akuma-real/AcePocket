...
final databaseServerOptionsProvider =
    FutureProvider.autoDispose.family<List<DatabaseServer>, String>(
  (ref, type) async {
...
final serverDatabaseUsersProvider =
    FutureProvider.autoDispose.family<List<DatabaseUser>, int>(
  (ref, serverId) async {
    final repo = ref.watch(databaseRepoProvider);
...
final databaseUserDetailProvider =
    FutureProvider.autoDispose.family<DatabaseUser, int>((ref, id) {
  return ref.watch(databaseRepoProvider).getUser(id);
});

/// 单个数据库服务器详情（`GET /api/database_server/{id}`）。
final databaseServerDetailProvider =
    FutureProvider.autoDispose.family<DatabaseServer, int>((ref, id) {
